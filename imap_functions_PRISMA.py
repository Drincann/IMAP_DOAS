...
def load_met(set_inputs, set_flight, files):
    ...
    low_ch4 = [35  , 112.55, 400, 400, 400, 400, 400, 400, \
...
def load_hitran_solar(wave_class, met, files):
    ...
    if files.hitran_loaded:
        ...
        fetch('hitran/CH4' + files.rad_dir, 2, 1, HWVL_min, HWVL_max )
...
def load_raw_observations(files, set_inputs, wave_class, met, cs):
    ...
    #PRISMA hack dcusworth 1/29/2020
    #wvl_sel2 = wvl_sel[3:]
    #wvl_sel3 = np.append(wvl_sel2, np.array([False] * 3))
    #wvl_sel = wvl_sel3
...
def Make_Jac3(T, lcoefs, scoefs, wave_class, met, set_inputs, cs, sCH4, sH2O, sN2O):
    ...
    lval = np.polynomial.legendre.legval(hi_x, lcoefs)
    sval = np.polynomial.legendre.legval(wave_class.disp_x, scoefs)[0]
    F = Forward3(T, lval)
    ...
    for idx in range(set_inputs.layers_ch4):
        sPerturb = sCH4.copy()
        sPerturb[idx] += delt
        Fp = np.flip(Transmission(sPerturb, sH2O, sN2O, met, cs)) * lval
        K[:,ii] = F_lo((Fp-F) / delt, np.flip(wave_class.wvl_hi), wave_class.wvl_lo, set_inputs.fwhm)
        ii += 1
    ...
    for idx in range(set_inputs.layers_ch4):
        sPerturb = sH2O.copy()
        sPerturb[idx] += delt
        Fp = np.flip(Transmission(sCH4, sPerturb, sN2O, met, cs)) * lval
        K[:,ii] = F_lo((Fp-F) / delt, np.flip(wave_class.wvl_hi), wave_class.wvl_lo, set_inputs.fwhm)
        ii += 1
    ...
    for idx in range(set_inputs.layers_ch4):
        sPerturb = sN2O.copy()
        sPerturb[idx] += delt
        Fp = np.flip(Transmission(sCH4, sH2O, sPerturb, met, cs)) * lval
        K[:,ii] = F_lo((Fp-F) / delt, np.flip(wave_class.wvl_hi), wave_class.wvl_lo, set_inputs.fwhm)
        ii += 1


    #Jacobian with respect to spectral shift
    dDispersion = scoefs * 1e-6 
    for idx in range(len(dDispersion)):
        new_poly = scoefs.copy()
        new_poly[idx] += dDispersion[idx]
        new_wvl = wave_class.wvl_lo * \
            (np.polynomial.legendre.legval(wave_class.disp_x, new_poly)[0] / wave_class.sval)
    ...
    if set_inputs.do_legendre:
        lperturb = lcoefs * 1e-6
        for idx in range(len(lperturb)):
            new_l = lcoefs.copy()
            new_l[idx] += lperturb[idx]
            new_lval = np.polynomial.legendre.legval(hi_x, new_l)
            Fp = Forward3(T, new_lval)
            K[:, ii] = F_lo((Fp - F) / lperturb[idx], np.flip(wave_class.wvl_hi), wave_class.wvl_lo, set_inputs.fwhm)
            ii += 1

    else:
        for idx in range(len(lcoefs)):
            K[:, ii] = (idx+1) * (lcoefs[idx]) * (wave_class.wvl_lo**idx)
            ii += 1
...
#Do retrieval for single pixel
def retrieve_scene(Y, set_inputs, wave_class, met, cs):
    ...
    hi_x = np.linspace(-1, 1, len(wave_class.wvl_hi))
    lo_x = interpolate.interp1d(np.flip(wave_class.wvl_hi), hi_x)(wave_class.wvl_lo)
    #fit_sel = (wave_class.wvl_pos >= wave_class.hitran_wvl[0]) & \
    #                       (wave_class.wvl_pos <= wave_class.hitran_wvl[1])
    #fit_lo = wave_class.wvl_pos[fit_sel]
    #fit_lox = interpolate.interp1d(np.flip(wave_class.wvl_hi), hi_x)(fit_lo)

    if set_inputs.do_legendre:

        #Only where there are little CO2 absorptions
        ...
        #lfit = np.polynomial.legendre.legfit(fit_lox, Y_full[fit_sel], init_k)
        lval = np.polynomial.legendre.legval(hi_x, lfit)
        lval_lo = np.polynomial.legendre.legval(lo_x, lfit)

    else:
        near_zeros = np.where(wave_class.cs_low < np.percentile(wave_class.cs_low, 40))[0]
        lfit = np.polynomial.polynomial.polyfit(wave_class.wvl_lo[near_zeros], Y[near_zeros], init_k)
        lval = np.polynomial.polynomial.polyval(wave_class.wvl_hi, lfit)
        lval_lo = np.polynomial.polynomial.polyval(wave_class.wvl_lo, lfit)
    ...
    xa_full = np.append(s1, \
                np.append(s2, \
                np.append(s3, \
                np.append(wave_class.shift_coef, lfit))))
    ...
    sig_prior[(lCH4+lH2O):(lCH4+lH2O+lN2O)] *= 1e-10 #N2O prior
    sig_prior[lCH4+lH2O+lN2O] *= 5e-10 #Shift prior
    ...
    invSe = np.diag([1/noise_val**2] * len(Y))
    ...
    #Run Forward model
    ilval = np.polynomial.legendre.legval(hi_x, ix[-len(lfit):])
    ...
    #Create Jacobian
    K = Make_Jac3(T, lcoefs=ix[-len(lfit):], scoefs=np.array([ix[lCH4+lH2O+lN2O]]), \
    ...
    lval_hat = np.polynomial.legendre.legval(hi_x, x_1[-len(lfit):])
...
def retrieve_scene2(Y, set_inputs, wave_class, met, cs):
    ...
    if set_inputs.do_legendre:
        ...
        #near_zeros = np.where(wave_class.cs_low < np.percentile(wave_class.cs_low, 30))[0]
        #zero_wvls = [1904.7063, 1914.1345, 1958.417, 2010.4419, 2069.5425, 2077.779, 2086.1228, 2094.4172, 2102.545]
        zero_wvls = [1904, 1914, 1923, 1932, 1984, 2036, 2094, 2102]#This one worked for hunter
        #zero_wvls = [1914, 1923, 1932, 1984, 2036, 2044, 2094, 2102, 2110, 2118]
        near_zeros = []
    ...
    #T_lo = F_lo(T, np.flip(wave_class.wvl_hi), wave_class.wvl_lo, set_inputs.fwhm)
    #Fa1 = T_lo * lval_lo
